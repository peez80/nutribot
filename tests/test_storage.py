...
@patch('app.storage.datetime')
@patch('builtins.open', new_callable=mock_open)
def test_create_session(mock_file, mock_datetime, mock_uuid, mock_makedirs):
...
def test_get_sessions(mock_file, mock_isfile, mock_listdir, mock_exists):
    ...
    # Mock reading two session files
    ...
    # handle reading files in sequence
    ...
    # Should be sorted newest first (session2 then session1)
...
def test_save_session_message(mock_exists, mock_file):
    ...
    # First read for getting current state, then write (dump) creates new calls to write
    mock_file.return_value.read.return_value = session_data
    
    save_session_message("123", {"text": "Hello", "is_user": True})
...
def test_update_session_title(mock_exists, mock_file):
    ...
    update_session_title("123", "New Title")
